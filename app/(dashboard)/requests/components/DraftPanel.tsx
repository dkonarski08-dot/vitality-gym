            ...
            {items.map((item, idx) => (
              <div key={idx} className="bg-white/[0.02] rounded-lg px-3 py-2 group">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      className="w-6 h-6 rounded bg-white/5 text-white/40 hover:text-white flex items-center justify-center text-xs transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      −
                    </button>
                    <input
                ...
                <input
                  type="text"
                  value={item.note ?? ''}
                  onChange={e => onUpdateNote(idx, e.target.value || null)}
                  placeholder="Бележка..."
                  className="w-full mt-1 bg-transparent text-[10px] text-white/40 focus:text-white/60 placeholder:text-white/20 focus:outline-none opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity"
                />
              </div>
            ))}
          </div>
